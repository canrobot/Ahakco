/*************************************************************************
* File Name          : ahakco_firmware.ino
* Author             : shin kyung man
* Updated            : shin kyung man
* Version            : V1.0
* Date               : 10/06/2017
* Description        : Firmware for 아두이노로 학급 코딩하기 with Scratch.  
  ...
  switch(device){
   case MOTOR:{
     ...
     if(val>=0&&val<=180&&val1>=0&&val1<=180){
        servo1.attach(7); 
        servo1.write(180-val); 
        servo2.attach(8); 
        servo2.write(val1); 
        if(val == 90 ){ servo1.attach(100); }
        if(val1 == 90 ){ servo2.attach(100); }
     }
   }
    break;
    case JOYSTICK:
